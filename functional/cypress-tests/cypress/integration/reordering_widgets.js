...
describe('Reordering widgets', () => {
  ...
  it("Logged in user can reorder widgets, logged out user can't", () => {
    validateOrder('Verifying the initial order', [firstWidgetsTitle, secondWidgetsTitle, thirdWidgetsTitle]);

    third.move(firstWidgetsTitle);

    validateOrder('Verifying if reordering widgets works', [thirdWidgetsTitle, firstWidgetsTitle, secondWidgetsTitle]);

    first.move(thirdWidgetsTitle);

    validateOrder('Verifying if reordering widgets works', [firstWidgetsTitle, thirdWidgetsTitle, secondWidgetsTitle]);

    cy.logout();
    second.move(firstWidgetsTitle);

    validateOrder('Verifying if user is unable to reorder widgets when logged out', [
      firstWidgetsTitle,
      thirdWidgetsTitle,
      secondWidgetsTitle,
    ]);
