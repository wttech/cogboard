...
function fillWorldClock() {
  ...
  cy.get('[id="select-content.timeZoneId"]').click();
  cy.contains(worldClock.timezone).click();
  //Change selector (add data-cy in markup)
  cy.get('[id="select-content.dateFormat"]').click();
  //Change selector (add data-cy in markup)
  cy.get(`[data-value="${worldClock.dateFormat}"]`).click();
  //Change selector (add data-cy in markup)
  cy.get('[id="select-content.timeFormat"]').click();
  //Change selector (add data-cy in markup)
  cy.get(`[data-value="${worldClock.timeFormat}"]`).click();
  if (randomBoolean()) {
    //Change selector (add data-cy in markup)
    cy.get('[name="content.displayDate"]').click();
  }
  if (randomBoolean()) {
    //Change selector (add data-cy in markup)
    cy.get('[name="content.displayTime"]').click();
  }
  //Change selector (add data-cy in markup)
  cy.get('[id="select-content.textSize"]').click();
  //Change selector (add data-cy in markup)
  cy.get(`[data-value="${worldClock.textsize}"]`).click();
}
