...
export function fillIframeEmbed() {
  cy.get('[data-cy="widget-form-content.url-input"]').type(
...
export function fillText() {
  cy.get('[data-cy="widget-form-content.text-input"]').type(Widgets.text.text);
  cy.get('[data-cy="widget-form-content.text-size-input"]').click();
  //Change selector (add data-cy in markup)
  cy.get('[data-value="h3"]').click();
  if (randomBoolean()) {
    cy.get('[data-cy="widget-form-content.is-vertical-input"]').click();
  }
}

export function fillWorldClock() {
  cy.get('[data-cy="widget-form-content.time-zone-id-input"]').click();
  cy.contains(Widgets.worldClock.timezone).click();
  cy.get('[data-cy="widget-form-content.date-format-input"]').click();
  //Change selector (add data-cy in markup)
  cy.get(`[data-value="${Widgets.worldClock.dateFormat}"]`).click();
  cy.get('[data-cy="widget-form-content.time-format-input"]').click();
  //Change selector (add data-cy in markup)
  cy.get(`[data-value="${Widgets.worldClock.timeFormat}"]`).click();
  if (randomBoolean()) {
    cy.get('[data-cy="widget-form-content.display-date-input"]').click();
  }
  if (randomBoolean()) {
    cy.get('[data-cy="widget-form-content.display-time-input"]').click();
  }
  cy.get('[data-cy="widget-form-content.text-size-input"]').click();
  //Change selector (add data-cy in markup)
