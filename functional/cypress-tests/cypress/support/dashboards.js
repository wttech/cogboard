...
Cypress.Commands.add('addDashboard', (dashboardName = "Dashboard" + Date.now().toString(), columns = "4", switchInterval = "10", expectedFailure = false) => {
    cy.get('[data-cy="navbar-show-drawer-button"]')
        .click();
...
Cypress.Commands.add('renewDashboards', (username=Cypress.env("username"), password=Cypress.env("password")) => {
  cy.request('post', 'http://localhost:8092/api/login', {username: username, password:password})
    .then((response) => {
