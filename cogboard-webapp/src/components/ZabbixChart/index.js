...
const ZabbixChart = ({ id, content }) => {
  ...
  useMemo(() => {
    const LABELS = Object.keys(content.history).map(label =>
      convertEpochToDate(label).toLocaleString()
    );
    ...
    setData({
      ...
      series: [getNumberOfElements(SERIES, setProgressSize.numberOfResults)]
    });
  ...
  const setBarColor = (value, maxValue, range) => {
    ...
    const percentageValue = checkMetricHasMaxValue
      ? calculatePercentageValue(value.y, maxValue)
      : value.y;
    let barColorStatus;
  ...
  const onDrawHandler = context => {
    ...
    if (context.type === 'label' && context.axis.units.pos === 'x') {
      ...
      const multilineText = Chartist.Svg('svg').foreignObject(
        textHtml,
        {
          x: context.x,
          y: context.y,
          width: context.axis.stepLength,
          height: 20
        },
        'ct-label ct-horizontal cta-end custom-label',
        true
      );
  ...
  return (
    <StyledZabbixChart data-axis-x="Date" data-axis-y={setAxisYTitle()}>
      <ChartistGraph
        listener={{
